...
const char * const BITCOIN_CONF_FILENAME = "skt.conf";
const char * const BITCOIN_PID_FILENAME = "sktd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "skt" is a composite category enabling all MonetaryUnit-related debug output
            if(ptrCategory->count(string("skt"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MonetaryUnitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MonetaryUnitCore
    // Mac: ~/Library/Application Support/MonetaryUnitCore
    // Unix: ~/.sktcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MonetaryUnitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MonetaryUnitCore";
#else
    // Unix
    return pathRet / ".sktcore";
#endif
#endif
}
