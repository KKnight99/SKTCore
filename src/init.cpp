...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MonetaryUnit specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that MonetaryUnit Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MonetaryUnit Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize MonetaryUnit Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single MonetaryUnit Core process is using the data directory.
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MonetaryUnit Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // ********************************************************* Step 11c: update block tip in MonetaryUnit modules
