...
const char* BIP71_MIMETYPE_PAYMENT = "application/skt-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/skt-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/skt-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the skt: URI contains a payment request, we are not able to detect the
...
//
// OSX-specific way of handling skt: URIs and PaymentRequest mime types.
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // skt: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid SKT address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom skt addresses are not supported
        ...
        // SKT amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
